package com.newink.aipassagecreator.agent.agents;
...
public class OutlineGeneratorAgent implements NodeAction {
    ...
    public Map<String, Object> apply(OverAllState state) throws Exception {
        ...
        // 记录原始返回内容用于调试
        log.info("OutlineGeneratorAgent LLM 返回内容长度: {}, 内容前100字符: {}",
        ...
        if (outlineResult == null || outlineResult.getSections() == null || outlineResult.getSections().isEmpty()) {
            log.error("OutlineGeneratorAgent 解析失败: outlineResult 为空或 sections 为空, content={}", content);
            throw new RuntimeException("大纲生成失败：LLM 返回内容解析为空");
        }
